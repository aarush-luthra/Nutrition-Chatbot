...
const API_BASE = '';  // Empty for same origin, or 'http://localhost:5000' for development
...
const state = {
    ...
    setupComplete: localStorage.getItem('fitbuddy_setup_complete') === 'true',
    userPreferences: JSON.parse(localStorage.getItem('fitbuddy_preferences') || '{}'),
};
...
function setupProfileSetupListeners() {
    // Setup option button clicks
    document.querySelectorAll('.setup-option').forEach(btn => {
        btn.addEventListener('click', () => {
            const field = btn.dataset.field;
            const value = btn.dataset.value;
            const container = btn.closest('.setup-options');
            ...
            if (isMultiSelect) {
                ...
                if (value === 'none' && btn.classList.contains('selected')) {
                    container.querySelectorAll('.setup-option').forEach(opt => {
                        if (opt !== btn) opt.classList.remove('selected');
                    });
                } else if (value !== 'none') {
                    // Deselect "None" if other option is selected
                    container.querySelector('[data-value="none"]')?.classList.remove('selected');
                }

                // Collect all selected values
                const selectedValues = [];
                container.querySelectorAll('.setup-option.selected').forEach(opt => {
                    selectedValues.push(opt.dataset.value);
                });
                state.userPreferences[field] = selectedValues;
            } else {
                ...
                state.userPreferences[field] = value;
            ...
            // Save preferences
            localStorage.setItem('fitbuddy_preferences', JSON.stringify(state.userPreferences));
        });
    });
}
...
function nextSetupStep() {
    ...
    const nextStep = document.querySelector(`.setup-step[data-step="${currentSetupStep}"]`);
    if (nextStep) {
...
function showSetupSummary() {
    ...
    const labels = {
        age: { 'under18': 'Under 18', '18-25': '18-25', '26-35': '26-35', '36-45': '36-45', '46-55': '46-55', '55+': '55+' },
        gender: { 'male': 'Male', 'female': 'Female', 'other': 'Other', 'prefer_not': '' },
        goal: { 'lose_weight': 'Lose weight', 'gain_weight': 'Gain weight', 'build_muscle': 'Build muscle', 'eat_healthier': 'Eat healthier', 'manage_condition': 'Health condition' },
        diet: { 'none': '', 'vegetarian': 'Vegetarian', 'vegan': 'Vegan', 'eggetarian': 'Eggetarian', 'nonveg': 'Non-veg', 'keto': 'Keto' },
        activity: { 'sedentary': 'Sedentary', 'light': 'Light activity', 'moderate': 'Moderate', 'very': 'Very active' }
    };

    Object.entries(state.userPreferences).forEach(([key, value]) => {
        if (value && labels[key]) {
            const displayValue = Array.isArray(value)
                ? value.filter(v => v !== 'none').map(v => labels[key]?.[v] || v).join(', ')
...
function completeSetup() {
    ...
    localStorage.setItem('fitbuddy_setup_complete', 'true');
    hideProfileSetup();
...
async function syncPreferencesToBackend() {
    try {
        ...
        console.log('Could not sync preferences:', error);
    }
}
...
function addBotMessage(text, isError = false) {
    ...
    <div class="message-avatar">
      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.5" stroke-linecap="round" stroke-linejoin="round">
        <path d="M4 11h16a1 1 0 0 1 1 1v.5c0 1.5-2.52 5.5-9 5.5s-9-4-9-5.5V12a1 1 0 0 1 1-1z"/>
        <path d="M12 4c-1.197 0-2.5.4-3.5 1.2-1.1.867-1.5 2-1.5 2.8 0 .733.6 1.4 1.5 2"/>
        <path d="M12 4c1.197 0 2.5.4 3.5 1.2 1.1.867 1.5 2 1.5 2.8 0 .733-.6 1.4-1.5 2"/>
        <path d="M12 4v7"/><path d="M7 20h10"/><path d="M9 20v-2"/><path d="M15 20v-2"/>
      </svg>
    </div>
